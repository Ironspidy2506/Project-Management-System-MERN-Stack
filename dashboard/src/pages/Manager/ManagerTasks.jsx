...
const ManagerTasks = () => {
  ...
  const handleFetch = async () => {
    if (viewBy === "date" && date) {
      try {
        const { data } = await axios.get(
          `https://project-management-system-mern-stack.vercel.app/api/tasks/get-date-wise/${date}`,
          { headers: { mtoken } }
        );
      ...
      try {
        const { data } = await axios.get(
          `https://project-management-system-mern-stack.vercel.app/api/tasks/get-month-wise/${month}/${year}`,
          { headers: { mtoken } }
        );
